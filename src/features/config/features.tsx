import { FeaturePage } from '@/pages/FeaturePage';
import { NotFound } from '@/pages/NotFound';
import type {
  FeatureCategoryKey,
  FeatureDisplayData,
  FeatureItemInput,
  FeatureKey,
  FeaturesConfig,
  RouteList,
} from '@/shared/types';
import { OnlyToggle } from '../css';
import { CharacterCounter, TagInput } from '../javascript';

const toKebabCase = (text: string) => text.toLowerCase().replace(/\s+/g, '-');

const generatePath = (category: string, day: number, name: string): string => {
  const kebabName = toKebabCase(name);
  return `/${category}/day-${day}-${kebabName}`;
};

const createFeatureItems = (
  category: FeatureCategoryKey,
  items: FeatureItemInput[],
) => {
  return items.map((item) => ({
    ...item,
    path: generatePath(category, item.day, item.name),
  }));
};

const jsItems: FeatureItemInput[] = createFeatureItems('js', [
  {
    day: 1,
    name: 'Show and Hide Password Input',
    component: () => <></>,
    isAble: false,
  },
  { day: 2, name: 'Custom Dropdown', component: () => <></>, isAble: false },
  {
    day: 3,
    name: 'Resizable Text Area',
    component: () => <></>,
    isAble: false,
  },
  {
    day: 4,
    name: 'Resizable Split Panels',
    component: () => <></>,
    isAble: false,
  },
  {
    day: 5,
    name: 'Character Counter',
    component: CharacterCounter,
    isAble: true,
  },
  { day: 6, name: 'Copy to Clipboard', component: () => <></>, isAble: false },
  { day: 7, name: 'Slugify Title', component: () => <></>, isAble: false },
  { day: 8, name: 'Tag Input', component: TagInput, isAble: true },
]);

const cssItems: FeatureItemInput[] = createFeatureItems('css', [
  { day: 1, name: 'Only Toggle', component: OnlyToggle, isAble: false },
]);

export const featuresCfg: FeaturesConfig = {
  js: {
    title: 'JavaScript',
    key: 'js',
    component: () => <FeaturePage featureKey="js" />,
    items: createFeatureItems('js', jsItems),
  },
  css: {
    title: 'CSS',
    key: 'css',
    component: () => <FeaturePage featureKey="css" />,
    items: createFeatureItems('css', cssItems),
  },
};

export const generateRoutes = (): RouteList => {
  const routes: RouteList = [];

  Object.entries(featuresCfg).forEach(([key, config]) => {
    routes.push({
      path: `/${key}`,
      component: config.component,
      crumb: config.title,
    });

    config.items.forEach((item) => {
      routes.push({
        path: item.path,
        component: item.component,
        crumb: `Day ${item.day}: ${item.name}`,
      });
    });
  });

  routes.push({
    path: '*',
    component: NotFound,
    crumb: 'Not Found',
  });

  return routes;
};

export const getFeatureData = (
  featureKey: FeatureKey,
): FeatureDisplayData | null => {
  const config = featuresCfg[featureKey];
  if (!config) return null;

  return {
    title: config.title,
    items: config.items.map((item) => ({
      name: `Day ${item.day}: ${item.name}`,
      isAble: item.isAble,
      path: item.path,
    })),
  };
};
